...
float temp = 0;
  ...
  printf(" EX8 Processor Utilization Factor is:%f\n",temp);
}
...
int i;
	 ...
	 rdl[i]--;
      if(rdl[i] == 0 && ret[i] != 0){
        printf("Deadline of task %d missed,Task set is not schedulable by LLF\n",i);
        return(1);
      }
...
int ddl_missed;
  ...
  if (ddl_missed != 1){
    printf("Tasks are feasibile by LLF\n");
  }
